...
class ContextUnet(nn.Module):
    ...
    def forward(self, x, c, t, context_mask):
        ...
        cemb1 = self.contextembed1(c).view(-1, self.n_feat * 2, 1, 1) # C, 256, 1, 1
        temb1 = self.timeembed1(t).view(-1, self.n_feat * 2, 1, 1) # C, 256, 1, 1
        cemb2 = self.contextembed2(c).view(-1, self.n_feat, 1, 1) # C, 128, 1, 1
        temb2 = self.timeembed2(t).view(-1, self.n_feat, 1, 1) # C, 128, 1, 1

        # could concatenate the context embedding here instead of adaGN
        # hiddenvec = torch.cat((hiddenvec, temb1, cemb1), 1)

        up1 = self.up0(hiddenvec) # up1: C, 256, 7, 7; hiddenvec: C, 256, 1, 1
        # up2 = self.up1(up1, down2) # if want to avoid add and multiply embeddings
        up2 = self.up1(cemb1*up1+ temb1, down2)  # add and multiply embeddings ... C, 128, 14, 14
        up3 = self.up2(cemb2*up2+ temb2, down1) # C, 128, 28, 28
        out = self.out(torch.cat((up3, x), 1)) # C, 1, 28, 28
        return out
...
class DDPM(nn.Module):
    ...
    def sample(self, x_i, c_i, context_mask, size, device, guide_w = 0.0, enable_store=True):
        ...
        for i in range(self.n_Timesteps, 0, -1):
            ...
            t_is = torch.tensor([i / self.n_Timesteps]).to(device) #[0.5]

            t_is = t_is.repeat(n_sample,1,1,1) # 125, 1, 1, 1
